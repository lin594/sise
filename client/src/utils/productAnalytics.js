...
export function trackProductEvent(name, fields = {}) {
    try {
        if (activeRequests >= 4)
            return;
        ...
        once.add(key);
        if (once.size > 256)
            once.delete(once.values().next().value);
...
export function openProductSession(invited) {
    trackProductEvent("app_open", { id: productVisitId(), persistent: hasPersistentBrowserStorage() });
    if (invited)
        trackProductEvent("invite_open", { id: productVisitId(), mode: "friends" });
}
export function beginProductMode(mode) {
    try {
...
export function readyProductMode() {
    if (!modeAttempt)
        return;
    ...
    if (durationMs >= 0 && durationMs <= 600000)
        trackProductEvent(modeEvent(attempt.mode), { id: attempt.id, mode: attempt.mode, outcome: "ready", durationMs });
}
export function failProductMode() {
    if (!modeAttempt)
        return;
    const attempt = modeAttempt;
    modeAttempt = null;
