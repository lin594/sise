...
function normalizeSnapshot(next) {
    ...
    return {
        ...
        previousWinnerId: String(rawState?.previousWinnerId ?? ""),
        previousWinnerName: String(rawState?.previousWinnerName ?? ""),
        previousHuType: String(rawState?.previousHuType ?? ""),
...
export function useRoom(playerName = "Player") {
    ...
    let connectionProbeInterval = null;
    let connectionProbeTimeout = null;
    ...
    function failStaleConnection(targetRoom, reason) {
    ...
    function sendConnectionProbe(targetRoom) {
        if (room.value !== targetRoom ||
            !connected.value ||
    ...
    function startConnectionProbes(targetRoom) {
        clearConnectionProbeTimers();
        sendConnectionProbe(targetRoom);
        if (room.value !== targetRoom || !connected.value) {
            return;
        }
        connectionProbeInterval = window.setInterval(() => sendConnectionProbe(targetRoom), CONNECTION_PROBE_INTERVAL_MS);
    }
    ...
    function buildStateSyncFingerprint(snapshot) {
        ...
        return [
            ...
            snapshot.previousWinnerId ?? "",
            snapshot.previousWinnerName ?? "",
            snapshot.previousHuType ?? "",
    ...
    async function connect(options) {
        ...
        try {
            ...
            joined.onMessage("connection_probe_ack", (payload) => {
                if (!isCurrentJoinedRoom()) {
                    return;
                }
            ...
            joined.onMessage("lobby_presence", (payload) => {
                if (!isCurrentJoinedRoom()) {
                    return;
                }
    ...
    function declareFish(fishCardIds) {
        declareError.value = "";
    ...
    function claimSeat(seatIndex) {
        joinError.value = "";
        return safeRoomSend("claim_seat", { seatIndex });
    }
