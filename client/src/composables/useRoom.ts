...
export function useRoom(playerName = "Player") {
  ...
  function primeQuickPhraseAudio(): void {
    if (quickPhraseAudioPrimed || quickPhraseMuted.value || quickPhrases.length === 0) return;
  ...
  function buildStateSyncFingerprint(snapshot: RoomStateSnapshot | null): string {
    ...
    const cardMark = (card: Card | null | undefined) => card
      ? [card.id, card.color, card.type, card.source ?? "", card.isResponseCard ? "1" : "0"].join(":")
      : "";
    const cardListMark = (cards: Card[] | undefined) => (cards ?? []).map(cardMark).join(",");
  ...
  function applySnapshot(next: unknown, source: "schema" | "explicit" = "schema") {
    ...
    const hasRevisionField = Boolean(
      rawSnapshot &&
  ...
  function debugApplyRoomSnapshot(
    patch: Partial<RoomStateSnapshot> & Record<string, unknown>,
    source: "schema" | "explicit" = "explicit",
  ): void {
    if (!state.value) {
      return;
    }
  ...
  function claimSeat(seatIndex: number): boolean {
    joinError.value = "";
