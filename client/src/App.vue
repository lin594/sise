...
import { useRoomRecovery } from "@/composables/useRoomRecovery";
...
const {
  ...
  nowMs,
  matchSecondsLeft
} = useLobbyPresentation({ state, players, connected, mySeatId, matchClockSync, isWaiting, isHost, mePlayer, hasLobbySession, enteringLobby, roundStartPending, selectedLobbyMode, pendingPracticeAutoStart, lobbyReadyPending });
...
const {
  confirmingResumeAbandon,
  syncExitButtonRef,
  resumeAbandonDialogRef,
  resumeAbandonCancelRef,
  bootstrapRoomEntry,
  syncCanRetry,
  syncScreenCopy,
  syncCancelDialogCopy,
  requestResumeAbandon,
  cancelResumeAbandon,
  confirmResumeAbandon,
  trapResumeAbandonFocus
} = useRoomRecovery({
  state, connected, connect, retryConnection, connectionState, joinError, acceptedStateRevision, mySeatId,
  entryName, entryInviteRoomId, enteredFrontLobby, enteringLobby, restoringStoredSession, joiningFriendInvite, startingRoomMode, globalError, nicknameHistory, storedEntryNameAtBoot, nicknameHistoryAtBoot, generateRandomNickname, enterLobby, privateHandSynchronized, returnToModeSelectionFromRoom: () => returnToModeSelectionFromRoom()
});
    ...
    const resolveTarget = () =>
      returnTarget?.isConnected && !returnToGameSettings
        ...
        : document.querySelector<HTMLElement>(
          "[data-testid='lobby-rules']:not(:disabled), [data-testid='game-settings']:not(:disabled), [data-testid='confirm-declaration']:not(:disabled), [data-testid='open-rules']:not(:disabled), [data-testid='login-submit'], .reset-btn",
        );
