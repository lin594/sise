...
说明：
- 前端 dev server 已默认监听 `0.0.0.0`，可被局域网设备访问。
- 前端默认会连接 `ws://<当前访问主机>:2567`，不再写死 `localhost`。
...
1. 电脑和手机连接同一 Wi-Fi。
2. 电脑执行 `ipconfig`，找到当前网卡 IPv4（例如 `192.168.1.23`）。
3. 手机浏览器访问：`http://192.168.1.23:5173`。
...
### 响应阶段（playing 内）
- `collective`：集体轮询阶段，仅 `胡/开/碰/过`。
- `local_upper`：本家专属（目标牌来自上家），仅 `吃/过`；过后摸新牌。
- `local_draw`：本家专属（目标牌来自摸牌），仅 `吃/过`；过后传下家。
