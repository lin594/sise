...
基于 `Colyseus + Vue 3 + TypeScript` 的四色牌项目。当前同时支持 **单人练习** 和 **好友同桌**：可以创建私有房、复制链接邀请朋友自由选座，并由房主在空座添加不同强度的机器人。

联机匹配、账号体系和公开房间大厅等功能目前只保留了部分前端入口或代码结构，尚未实现为可用功能。文档和代码出现冲突时，优先以当前代码和测试为准；规则歧义见 `docs/DOCS_CODE_GAPS.md`。
...
- 好友私房、邀请链接、四席自由选座与房主管理。
- 每座独立的 `0–100` 机器人强度；策略会评估牌型、组合收益、暴露代价和公开牌风险。
...
- `waiting`：等待大厅。好友房在四席由在线真人或配置机器人坐满后可启动。
...
- BOT 补位：单人练习自动补位；好友房由房主逐座添加并配置强度。
- 托管重连：等待大厅掉线保留座位 60 秒；对局中立即由 BOT 托管，同 token 重连可恢复座位。
- 房主转让：房主掉线时权限转给座位顺序最靠前的在线真人，权限与座位号不绑定。
...
- `LOBBY_SEAT_HOLD_MS`：等待大厅真人掉线后的座位保留时间。
- `WAITING_ROOM_IDLE_MS` / `ACTIVE_ROOM_IDLE_MS`：全员离线后的房间回收时间。
