...
- 许可证：维护者已在 #33 明确整体采用 AGPL-3.0，音频自行录制、SVG 等由 Codex 生成；已落实 LICENSE 和来源清单。
- 交付：每项独立 PR，当前提交 CI gate 通过后合并；部署、真机验证与代码合并分别记录。

任务与当前合并证据见 [产品化进度](docs/PRODUCT_ROADMAP.md)，验证环境、失败修复与验收边界见 [本轮验证记录](docs/validation/PRODUCTIZATION_2026-09-10.md)。

实现范围覆盖规则与版本恢复、匿名指标、教学/提示/邀请/复玩、文化页、App 八个领域拆分以及先审计后迁移生成文件。具体状态以产品化进度表的实际 PR 为准；规则与许可证已确认。真实设备、读屏与本地牌友验收迁入 #74；生产任务 #73 已完成，运行版本 c7d8ad2，统计启用与受控聚合增量已验证；最新证据以产品化进度为准。两个 milestone 按实际验收进度更新。
