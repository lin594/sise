...
客户端不能只凭公开回合状态开放操作。进入声明或对局后，必须以本人公开 `handCount` 与私有手牌长度相等作为私有状态就绪条件；数量不一致时冻结选牌和动作，并每秒请求一次 `sync_state` 与私有状态补拉。开局发牌展示动画尚未结束时也保持冻结。发牌只由服务端进入正式 `DEALER` 阶段触发一次；组件首次挂载若正处于翻定庄牌阶段，只恢复对应仪式画面，不得自行再启动发牌。这样即使 Schema 公共补丁先于同一时点的私有快照触发渲染，玩家也不会在半份手牌上操作或看到两轮发牌。
...
服务端先把状态切到结束阶段，再广播并保存权威 `round_result`。客户端可以立即显示“正在整理本局得分”，但必须等包含四家明细的 `round_result` 到齐后，才允许后续操作，避免状态补丁与结果消息先后到达时跳过结算。结果到齐后，客户端把固定的本人输赢、独立滚动的玩家明细和固定操作分层；紧凑屏默认折叠每家长明细，桌面默认展开，但两者读取同一份权威结果。好友房存在其他真人时，客户端在发送 `next_round` 前增加可取消、约束焦点的全桌影响确认；练习房或只有房主与配置机器人的桌直接发送。`next_round` 与 `return_lobby` 以互斥的本地提交状态等待服务端离开 `ended`，期间只允许一条整桌命令；大厅错误、断线、换房会立即解除，8 秒没有权威变化则恢复重试。快速桌隐藏整桌生命周期控制，每名真人通过客户端先执行个人 `leaveRoom()`，清理旧房凭证后重新 `joinOrCreate`；旧桌不受影响。任何真人仍可以从客户端个人退出，进行中的空座由机器人继续托管。
