...
等待阶段的好友房房主可发送 `dissolve_room`。服务端再次校验房型、阶段与房主身份，向全部连接广播 `room_dissolved` 后清空座位 token，并用终止关闭码结束房间。客户端把该消息作为不可重连事件，作废当前连接代次、清除凭证与 URL 房号并返回模式大厅；普通个人离房仍沿用原逻辑，不会关闭其他玩家的桌。
...
客户端后端地址由统一解析器生成：HTTPS 页面默认使用 HTTPS API 与 WSS，HTTP 页面默认使用 HTTP API 与 WS；显式配置其中一个 `VITE_SERVER_*` 地址时，另一条链路从同一主机、端口和路径推导，并清除查询串、片段及尾部斜杠。正式环境仍建议显式写出两项，Traefik 终止 TLS 后把 HTTPS/WSS 转发到容器内同一个 HTTP 服务；iMac 带端口试玩继续走 HTTP/WS，不参与 TLS 配置。
