...
快速桌自动领取第一个空闲固定座位。首位真人建立 `matchStartsAt`，后续加入不会延后它，四名真人到齐只会把它提前到短暂展示窗口。截止时服务端先验证所有保留真人都在线，再把 `matchOpen` 设为 false、补齐标准机器人并进入开局；有保留真人断线时维持原截止时间但暂停执行。四个逻辑座位已被占满时，即使其中一个连接暂时离线，也必须从公开匹配结果中移除该桌，避免第五人被送入后才遭拒绝。快速桌拒绝选座、计分修改、机器人管理、整桌下一局和整桌返厅消息。
