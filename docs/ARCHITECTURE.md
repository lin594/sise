...
全局响应计时接口沿用 `decisionTimer.totalMs` 和 `endsAt`：前者为本轮启动时确定的 10000／3000ms，后者为固定共享截止点；客户端不得按轮询重建倒计时。可响应真人不包含机器人、断线或主动托管玩家。房间快照保存截止点，旧快照从公开 `responseEndsAt` 兼容恢复。进入本地操作后重新开启普通计时（默认 30 秒）；互动短句仍独立使用 3 秒窗口。
