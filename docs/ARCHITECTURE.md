...
可见动作动画使用“局数、版本、动作码、目标牌 ID”以及同局语义键去重；版本较新但语义相同的迟到消息也不能重播已经展示的出牌、吃碰开或摸牌。四条流水在组件生命周期内保持固定 DOM 容器，数据归属来自排序后的座位环；方向偏好只镜像内容位置。当前回合和流水强调使用静态标记，持续动画仅允许合成层的 `transform` 与 `opacity`，不动画化布局尺寸、`filter` 或 `box-shadow`。
