...
新牌局动画使用服务端公开 tableTransitions 有序事件和 presentationUntil 截止时间；Schema 用 tableTransitionsJson 保存同一数据，恢复快照一并持久化。事件仅包含已公开移动牌、起终位置、局号、序号和时间，最多保留本局最近 32 条。客户端按绝对时间绘制，过期和重复消息不重播；展示间隔内服务端拒绝操作并延后机器人与响应计时，终局通过私有 pendingPresentationEnd 恢复延迟结算。无客户端的模拟牌局跳过展示延迟。旧协议仍保留动作语义键去重。四条流水在组件生命周期内保持固定 DOM 容器，数据归属来自排序后的座位环；方向偏好只镜像内容位置。当前回合和流水强调使用静态标记，持续动画仅允许合成层的 `transform` 与 `opacity`，不动画化布局尺寸、`filter` 或 `box-shadow`。
