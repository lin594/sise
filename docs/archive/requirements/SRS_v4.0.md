...
#### 亮标识阶段（仅开局1次）
1. **声明暗坎数量**  
   - 系统默认选择扣除已亮鱼牌后的最大可声明数，玩家可通过 `0…N` 数量按钮调整
   - **含义**：承诺本局游戏过程中，手牌中**始终保留至少N组未开、未亮鱼的暗坎**  
2. **亮鱼（可选）**  
   - 系统默认选中全部合法鱼，玩家按完整牌组点选或取消：
     - 普通鱼：4张同色同字（如4张“黄车”）  
     - 金条鱼：4张或5张金条（任意组合）  
   - 5张金条可成鱼时默认选择5张方案，4张与5张方案互斥
   - **效果**：亮出牌**永久锁定**，不可用于开/吃/碰；胡牌时直接计分  
3. **确认与超时**
   - 手牌以不可编辑的单行预览显示；青色标记已选亮鱼，橙色标记可组成暗坎的牌
   - 玩家一次确认后不可撤销；超时未确认仍由服务端按系统建议自动提交
...
> - **操作面板**：自己手牌右侧固定操作坞；响应阶段显示动作，弃牌阶段显示“先选牌/出牌”
...
| **待响区牌** | • 作为中央唯一主视觉• 不显示抽牌者姓名• 无待响牌时不放置空占位框 | 与牌堆组成固定双槽：左侧牌堆、右侧待响，整个组合居中；待响出现或消失时牌堆不移动 |
| **牌堆造型** | • 八张牌背象形叠放• 牌背为红色• 单牌细长且上下两端为圆头• 顶层显示准确余量 | 不使用普通扑克牌的宽矩形、蓝色背纹或方角造型 |
| **对手手牌数** | • 三位其他玩家的剩余手牌数始终显示在姓名旁• 所有尺寸均不可隐藏 | 使用独立“数字+张”徽标，牌组和暗坎信息不重复手牌数 |
| **声明面板** | • 亮鱼按整组选择• 暗坎用数量按钮选择• 默认采用系统建议• 确认按钮保持吸底 | 青色表示亮鱼，琥珀色表示暗坎；按钮实时汇总声明结果 |
| **手牌与出牌** | • 小屏单行横滑• 点击、重复点击和双击只更新选中牌• 右侧操作坞确认出牌• 将牌/金条不可选 | 未选牌显示禁用“先选牌”，选中后高亮“出牌”；提交后播放动画并防重复请求 |
| **牌面显示** | • “我的牌”和“桌面牌”分别支持大字/自适应/长牌• 默认自适应 | 小屏自适应为正向大字，标准桌面自适应为上下双字；设置即时生效并持久化 |
| **牌局工具** | • 顶部专用控制栏在声明、对局、结算阶段常驻• 左侧仅显示“四色牌”，右侧为设置与个人退出• 二级设置包含牌面、座位方向和规则 | 顶栏不承载牌局信息；个人退出确认后返回模式大厅且不自动重连 |
| **座位方向** | • 可选顺时针（下家在左）或逆时针（下家在右）• 默认逆时针 | 只镜像本地左右玩家及其牌组、流水、提示和动画，不改变服务器座位和行动顺序 |
| **胡牌提示** | • 胡牌时弹出结算面板（半透明遮罩）• 显示拆解方案（高亮有效牌组） | 胜利音效 + 粒子动画 |

### 4.3 横屏画布与适配
| 设备 | 要求 | 实现方案 |
|------|------|----------|
| **手机** | **横屏画布** | • 系统竖屏时交换有效宽高并将应用旋转90°，不显示方向遮罩• 系统锁定竖屏时仍可完整触控• 667×375 下自适应手牌宽约40–44px、间隔约2px、首屏约9–10张、字形不小于约22px• 操作按钮依有效空间自适应，不设全局固定尺寸 |
| **平板** | 横屏优先 | 按有效宽高选择布局密度，界面元素等比调整 |
...
| 有效横屏尺寸 | 布局调整 |
|----------|----------|
| 宽度 > 960px 且高度 > 500px | 标准牌桌布局，与移动端共用精简中央信息结构 |
| 宽度 ≤ 960px 或高度 ≤ 500px | 专用顶部控制栏、三列底部控制带、40–44px单行横滑手牌、右侧确认出牌/3×2响应操作坞 |
| 宽度 ≤ 720px 或高度 ≤ 380px | 超紧凑信息密度，流水只显示最近8张 |
...
| 用例 | 操作步骤 | 预期结果 |
|------|----------|----------|
| **竖屏横向画布** | 手机竖屏或锁定竖屏打开 | 无方向遮罩，应用以交换宽高后的横屏画布旋转显示并可操作 |
| **按钮点亮** | B待响区“黄车”，A手无匹配 | A界面“胡/开/碰”全灰，“过”可点 |
| **弃牌区显示** | A弃3张牌 | B/C/D界面均可见A弃牌区3张完整牌面 |
| **明示区高亮** | A吃”红车马炮架” | target牌有金色边框+★角标+脉冲动画 |
| **触屏优化** | 小尺寸手机点击“吃”或“出牌”按钮 | 控件在安全区内自适应，点击区域稳定且不挤占牌桌 |
| **确认出牌** | 反复点击或双击一张普通手牌 | 手牌不变化；选中高亮，只有点击右侧操作坞的“出牌”后才提交 |
| **中央精简** | 定庄后等待并出现待响牌 | 仅庄家姓名旁显示庄标与定庄牌；中央为固定的“左牌堆、右待响”双槽，组合整体居中，待响出现前后牌堆位置不变 |
| **对手手牌余量** | 观察三位其他玩家在摸牌、吃碰和出牌前后的座位信息 | 每位对手姓名旁持续显示准确的“n张”，小屏也不隐藏 |
| **牌面偏好** | 分别切换我的牌和桌面牌的大字/自适应/长牌 | 对应全部牌区即时更新，刷新后保留；旧偏好能迁移 |
| **座位方向** | 在顺时针与逆时针间切换 | 左右玩家及所属牌区镜像，对家和自己不动，实际当前玩家与行动顺序不变 |
| **个人退出** | 打开右上角退出，依次验证取消和确认 | 取消继续牌局；确认清除房间凭证、停止重连并返回模式大厅 |
...
| `useResponsiveViewport.ts` | 有效视口与手机横屏画布状态 | `window.innerWidth/innerHeight`、粗指针媒体查询 |
...
> - `client/src/components/`：前端组件（含操作面板/弃牌区）
