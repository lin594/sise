...
- 麻将模式所有牌面文字保持正向：四家流水、明示牌、自己的手牌、长牌上下文字及颜色辅助字；牌框保留座位朝向。飞牌复制目标字向，切回经典恢复原有上下字向。
- 左、右、对家旋转飞牌与最终牌面的矩形误差 ≤ 1px；动画结束前不切换实际布局。

## 结果

- 牌面文字正向修复：构建通过，合入颜色辅助空间修复后，Chrome / WebKit 拥挤流水、旋转飞牌、十二组吃牌、颜色辅助字无遮挡及切回经典字向恢复共 18 项通过；手机和桌面截图已刷新。
...
- WebKit 缩放兼容修复（直接应用像素尺寸）后，十二组完整显示、极端滚动、跨皮肤布局切换及飞牌落点复跑 10 项通过。
- 压测夹具等待公开场景与私有手牌都到达后再覆盖快照，避免迟到的场景消息覆盖 21 张压测手牌；桌面密度和十二组显示双浏览器复跑 8 项通过。
