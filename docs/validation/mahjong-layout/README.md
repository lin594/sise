...
- 四家明示牌统一横向整组排列，多组从左到右、从上到下靠左换行；组间横向留白 6px 大于组内 1px。玩家姓名与牌组起点对齐，流水每行从左开始，减少扫视时的跳动。
- 移动端优先验证大字模式：640×350、667×375、844×390、932×430、390×844，覆盖 12 组完整显示和 28 组滚动，包含颜色辅助及吃牌飞行落点。
...
- 麻将模式所有牌面文字保持正向：四家流水、明示牌、自己的手牌、长牌上下文字及颜色辅助字；明示牌牌框也正向显示，流水牌框保留座位朝向。飞牌复制目标字向，切回经典恢复原有上下字向。
- 左、右、对家旋转飞牌与最终牌面的矩形误差 ≤ 1px；动画结束前不切换实际布局。

## 结果

- 横排阅读优化：构建通过；移动端大字优先的双浏览器检查 12 项通过，长牌兼容与两种牌型滚动/触摸补充回归 18 项通过、2 项跳过（WebKit 不支持 CDP 触摸注入）；新增横向整组和阅读顺序断言。
...
- [普通手机麻将桌（大字模式）](mahjong-large-844x390.png)
- [竖屏麻将桌](mahjong-large-390x844.png)
- [桌面手动选择麻将布局（大字模式）](mahjong-large-1440x900.png)
- [桌面自适应选择经典](adaptive-classic-desktop.png)
- [自己 12 组吃牌，首屏完整显示](twelve-eats-self-large-667x375.png)
- [对家 12 组吃牌，首屏完整显示](twelve-eats-top-large-667x375.png)
- [上家 12 组吃牌，手机竖屏完整显示](twelve-eats-left-large-390x844.png)
- [上家 12 组吃牌，手机横屏完整显示](twelve-eats-left-large-667x375.png)
- [下家 12 组吃牌，手机横屏完整显示](twelve-eats-right-large-667x375.png)
