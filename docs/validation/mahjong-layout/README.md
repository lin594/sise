...
- 最终 Chrome / WebKit 布局回归：29 项通过、2 项跳过；跳过项是 WebKit 不支持 CDP 的原生触摸注入测试，相同滚动区域已通过 WebKit 几何与访问检查。
...
PLAYWRIGHT_CHANNEL=chrome npx playwright test \
  ...
  --grep 'mahjong|adaptive is|all skins and layouts|other skins and layouts|touch swipes|twelve eats|keeps exposed cards'
```
