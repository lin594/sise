...
| D1 / D7 | 首次观察后第 1 / 7 个自然日出现 app_open 的可长期存储档案 / 同 cohort 的可长期存储档案；目标日结束且有当日事件采集记录才进入分母，不按累计留存统计 |
...
CLI 的 `retentionCoverage` 同时列出成熟/可测 cohort 数、排除人数和缺失目标日期。目标日没有采集记录（或只有后续回填的 cohort 计数）时，该 cohort 不计作零留存；全缺失时返回 null。目标日有部分记录也不能证明全天采集完整，报告仍只代表观察样本。
