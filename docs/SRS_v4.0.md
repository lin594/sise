...
| **待响区牌** | • 作为中央唯一主视觉• 不显示抽牌者姓名• 无待响牌时不放置空占位框 | 牌面轻微浮动动画，牌堆余量缩为角落徽标 |
| **声明面板** | • 亮鱼按整组选择• 暗坎用数量按钮选择• 默认采用系统建议• 确认按钮保持吸底 | 青色表示亮鱼，琥珀色表示暗坎；按钮实时汇总声明结果 |
| **手牌与出牌** | • 小屏单行横滑• 点击、重复点击和双击只更新选中牌• 独立“出牌”按钮提交• 将牌/金条不可选 | 选中牌上浮高亮；提交后播放出牌动画并防重复请求 |
| **牌局工具** | • 声明、对局、结算阶段常驻设置与个人退出• 牌面模式和规则位于二级设置• 退出需要确认 | 偏好本地持久化；个人退出后返回模式大厅且不自动重连 |
| **胡牌提示** | • 胡牌时弹出结算面板（半透明遮罩）• 显示拆解方案（高亮有效牌组） | 胜利音效 + 粒子动画 |

### 4.3 横屏画布与适配
| 设备 | 要求 | 实现方案 |
|------|------|----------|
| **手机** | **横屏画布** | • 系统竖屏时交换有效宽高并将应用旋转90°，不显示方向遮罩• 系统锁定竖屏时仍可完整触控• 手牌单行横滑，牌位节距约30–36px• 按钮依有效画布自适应，以稳定触控为准 |
...
| 宽度 ≤ 960px 或高度 ≤ 500px | 三列移动控制带、紧凑单行横滑手牌、确认出牌、3×2操作坞 |
...
| **确认出牌** | 反复点击或双击一张普通手牌 | 手牌不变化；选中高亮，只有点击独立“出牌”后才提交 |
| **中央精简** | 定庄后等待并出现待响牌 | 仅庄家姓名旁显示庄标与定庄牌；中央只有待响牌主视觉和牌堆角标 |
