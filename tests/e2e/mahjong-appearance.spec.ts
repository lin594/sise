...
for (const mode of ['long', 'large']) test(`mahjong ${mode} keeps crowded rivers central and cards accessible`, async ({ page }, info) => {
  ...
  await crowd(page);
  for (const [width, height] of [[667, 375], [844, 390], [932, 430], [390, 844], [1024, 768], [1440, 900]]) {
    await page.setViewportSize({ width, height });
...
async function manyMelds(page: Page, side: string) {
  await page.evaluate(side => {
    ...
    // 106 distinct physical cards: 21 in hand, 40 in rivers, 36 in the stressed
    // seat's 12 eats, and three in each other seat. No unbounded fake deck.
    const deck = ['red', 'yellow', 'green', 'white'].flatMap(color =>
      ['jiang', 'shi', 'xiang', 'ju', 'ma', 'pao', 'zu'].flatMap(type =>
    ...
    bridge.applyRoomSnapshot({ stateRevision: state.stateRevision + 100, phase: 'playing',
      ...
      players: state.players.map((p: any) => {
        const count = p.clientId === target ? 12 : 1;
        return { ...p, handCount: p.isBot ? 0 : 21, declaredKongs: 0, fishArea: [], pendingFishGroupSizes: [],
          discardPile: take(10), exposedArea: take(count * 3), exposedGroupSizes: Array(count).fill(3), exposedGroupKinds: Array(count).fill('chi') };
      }) }, 'explicit');
  }, side);
  const area = page.locator(side === 'self' ? '.self-groups-card' : `.player-${side} .group-block-list`);
  await expect(area.locator('.group-block')).toHaveCount(12);
  await expect(page.getByTestId('game-board')).toHaveAttribute('data-geometry-busy', 'false');
  await expect(page.locator('.deal-overlay')).toHaveCount(0);
  return area;
}

for (const mode of ['long', 'large']) test(`twelve eats at every seat remain individually accessible in ${mode} mode`, async ({ page }, info) => {
  await start(page, 'mahjong', mode);
  for (const side of ['self', 'left', 'top', 'right']) {
    ...
    for (const [width, height] of [[640, 350], [667, 375], [390, 844], [1440, 900]]) {
      await page.setViewportSize({ width, height });
      await page.waitForTimeout(250);
      // Check every group, not merely the existence of the first or last one.
      for (const group of await area.locator('.group-block').all()) {
        await group.evaluate(el => {
          const scroll = el.closest('.self-groups-card') ?? el.parentElement!;
          scroll.scrollLeft = (el as HTMLElement).offsetLeft;
          scroll.scrollTop = (el as HTMLElement).offsetTop;
        });
        const accessible = await group.evaluate(el => {
          const scroll = el.closest('.self-groups-card') ?? el.parentElement!;
        ...
        if (!accessible) {
          console.log('inaccessible group', await group.evaluate(el => ({ box: el.getBoundingClientRect().toJSON(), scroll: (el.closest('.self-groups-card') ?? el.parentElement!).getBoundingClientRect().toJSON(), cards: [...el.querySelectorAll('.mini-card')].map(card => card.getBoundingClientRect().toJSON()) })));
          await page.screenshot({ path: info.outputPath('inaccessible-group.png') });
        }
        expect(accessible, `${side} ${mode} ${width}x${height}`).toBe(true);
      }
