# 四色牌游戏 - 项目概览

## 📋 项目信息

- **项目名称**: 四色牌游戏 (Four-Color Card Game)
- **版本**: 2.0.0 (Colyseus重构版)
- **架构**: Colyseus + Vue 3 + TypeScript
- **状态**: ✅ **完全可玩**
- **许可**: 仅供学习和娱乐使用

## 🎯 项目状态

| 模块 | 状态 | 完成度 |
|------|------|--------|
| 后端架构 | ✅ 完成 | 100% |
| 前端架构 | ✅ 完成 | 100% |
| 游戏流程 | ✅ 完成 | 100% |
| UI/UX | ✅ 完成 | 100% |
| 文档 | ✅ 完成 | 100% |
| 部署配置 | ✅ 完成 | 100% |

**总体完成度**: **100%** ✅

## 🎮 完整功能清单

### 已实现 ✅ (全部完成)

#### 核心游戏系统
- ✅ **117张卡牌系统**
  - 7种角色 × 4种颜色 × 4张 = 112张
  - 5张金条牌
  
- ✅ **多人游戏支持**
  - 4人实时对战
  - WebSocket低延迟同步
  - 断线AI接管

- ✅ **完整游戏流程**
  - 定庄机制
  - 发牌系统
  - 声明暗坎
  - 打出/响应机制
  - 集体询问系统
  - 抓牌/过牌

#### 操作系统
- ✅ **吃操作（7种牌组）**
  - 车马炮架
  - 将士象架
  - 三异色卒
  - 四异色卒
  - 对子
  - 单将组
  - 单金条组

- ✅ **胡牌判定**
  - 递归拆解算法
  - 100%有效牌组验证
  - 单将/金条自动归属
  - 多方案选择

- ✅ **开操作**
  - 普通开（3+1）
  - 金条开

- ✅ **碰操作**
  - 2张相同牌+响应牌
  - 将/金条限制

#### 计分与结算
- ✅ **完整计分系统**
  - 所有牌组得分规则
  - 互结分计算
  - 胡牌得分
  - 总分统计

- ✅ **游戏结束**
  - 结算界面
  - 得分变化展示
  - 胜利者公告

#### 特殊规则
- ✅ 将/金条不可主动打出
- ✅ 响应优先级：胡>开>碰
- ✅ 吃操作前置
- ✅ 弃牌区全公开

#### 移动端优化
- ✅ 强制横屏模式
- ✅ 触屏友好
- ✅ 自适应布局

## 📚 文档导航

### 快速入门
- **[QUICKSTART.md](./QUICKSTART.md)** - 3步开始游戏 ⭐ 新手首选
- **[README.md](./README.md)** - 游戏规则说明

### 开发文档
- **[README-COLYSEUS.md](./README-COLYSEUS.md)** - 技术架构详解
- **[INSTALL-NEW.md](./INSTALL-NEW.md)** - 详细安装指南
- **[IMPLEMENTATION-MAP.md](./IMPLEMENTATION-MAP.md)** - SRS需求对照表

### 其他
- **[preview.html](./preview.html)** - 架构可视化预览

---

## 🚀 快速开始

```bash
# 1. 克隆项目
git clone https://github.com/lin594/sise.git
cd sise

# 2. 安装依赖
npm install

# 3. 启动开发服务器
npm run dev

# 4. 访问游戏
# 打开浏览器: http://localhost:3000
```

## 📦 项目结构

```
sise/
├── server/                 # Colyseus 后端
│   ├── src/
│   │   ├── schema/        # 游戏状态定义
│   │   ├── rooms/         # 游戏房间逻辑
│   │   ├── utils/         # 工具函数
│   │   │   ├── constants.ts    # 常量
│   │   │   ├── cardUtils.ts    # 卡牌工具
│   │   │   ├── validator.ts    # 游戏规则验证器 ✨
│   │   │   └── scoring.ts      # 计分系统 ✨
│   │   └── index.ts       # 服务器入口
│   └── package.json
│
├── client/                # Vue 3 前端
│   ├── src/
│   │   ├── components/   # Vue组件
│   │   │   ├── GameBoard.vue      # 游戏主界面 ✨
│   │   │   ├── PlayerArea.vue     # 玩家区域
│   │   │   ├── ActionPanel.vue    # 操作面板 ✨
│   │   │   ├── Card.vue           # 卡牌组件
│   │   │   └── OrientationGuard.vue # 横屏强制
│   │   ├── App.vue
│   │   ├── main.ts
│   │   └── style.css
│   └── package.json
│
├── docker-compose.yml     # Docker配置
├── package.json           # 根package.json
└── docs/                  # 文档（见上方文档导航）
```

✨ = 最近更新（完整游戏逻辑）

## 🛠️ 开发命令

```bash
# 同时启动前后端
npm run dev

# 仅启动后端
npm run dev:server

# 仅启动前端
npm run dev:client

# 构建生产版本
npm run build

# Docker部署
docker-compose up -d
```

## 🌐 端口配置

- **前端**: http://localhost:3000
- **后端**: ws://localhost:2567
- **健康检查**: http://localhost:2567/health

## 🎨 技术栈

### 后端
- **Colyseus** 0.15+ - 实时多人游戏框架
- **TypeScript** - 类型安全
- **Express** - HTTP服务器
- **Node.js** 18+ - 运行环境

### 前端
- **Vue 3** - 渐进式框架
- **TypeScript** - 类型安全
- **Vite** - 构建工具
- **Colyseus.js** - 客户端SDK

### 部署
- **Docker** - 容器化
- **Nginx** - 静态文件服务
- **Docker Compose** - 编排

## 📊 代码统计

```
Server:
- Schema: ~50 lines
- Room Logic: ~800 lines
- Validator: ~400 lines ✨ 新增
- Scoring: ~200 lines ✨ 新增
- Utils: ~150 lines

Client:
- Components: ~1200 lines
- Styles: ~400 lines
- Total: ~1600 lines

Documentation: ~20,000 words
```

## 🎯 核心算法

### 胡牌判定算法
使用递归回溯算法，将手牌拆解为有效牌组：
```typescript
function decomposeCards(cards: ICard[]): CardGroup[][] {
  if (cards.length === 0) return [[]]; // 拆解完成
  
  // 尝试各种牌组组合
  for (const checker of groupCheckers) {
    const combinations = getCombinations(cards, checker.size);
    for (const combo of combinations) {
      if (checker.check(combo)) {
        // 递归处理剩余牌
        const remaining = cards.filter(c => !combo.includes(c));
        const subResults = decomposeCards(remaining);
        // ... 组合结果
      }
    }
  }
  return results;
}
```

### 计分系统
双层结算：
1. **互结分**: 每两个玩家之间结算坎/开/鱼分数
2. **胡牌分**: 其他三家各付给胡牌者

```typescript
// 互结分：两两结算
for (let i = 0; i < players.length; i++) {
  for (let j = i + 1; j < players.length; j++) {
    const diff = score1 - score2;
    playerScores[i] += diff;
    playerScores[j] -= diff;
  }
}

// 胡牌分：三家付给胡牌者
for (const playerId of losers) {
  playerScores[playerId] -= huScore;
  playerScores[winnerId] += huScore;
}
```

## 🤝 参与贡献

欢迎提交Issue和Pull Request！

### 可优化方向 🔮

**体验优化**:
- [ ] AI策略增强（更智能的决策树）
- [ ] 音效系统（打牌/胡牌音效）
- [ ] 动画增强（牌移动动画）
- [ ] 亮鱼UI（拖拽亮鱼）

**功能扩展**:
- [ ] 暗坎校验（违规检测）
- [ ] 游戏回放系统
- [ ] 观战模式
- [ ] 排行榜系统
- [ ] 好友系统

**技术优化**:
- [ ] 单元测试覆盖
- [ ] E2E测试
- [ ] 性能优化
- [ ] 错误处理增强

## 📝 更新日志

### v2.0.0 (2026-02-03) - 完整可玩版 🎉
- ✅ 完整游戏逻辑实现
- ✅ 吃操作（7种牌组）
- ✅ 胡牌判定算法
- ✅ 开/碰操作
- ✅ 计分系统
- ✅ 游戏结束与结算
- ✅ 移动端横屏优化
- ✅ Docker部署支持
- ✅ 完整文档

### v1.0.0 (2026-01-15) - 纯前端版
- ⚠️ 已废弃（存在游戏流程错误）

## 🔗 相关链接

- **GitHub**: https://github.com/lin594/sise
- **Issues**: https://github.com/lin594/sise/issues
- **Colyseus文档**: https://docs.colyseus.io
- **Vue 3文档**: https://vuejs.org

## 📄 许可证

本项目仅供学习和娱乐使用。

---

**最后更新**: 2026-02-03  
**维护者**: lin594  
**状态**: ✅ 完全可玩！游戏已就绪，欢迎试玩！
